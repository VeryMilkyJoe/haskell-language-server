...
module Development.IDE.Core.Rules(
    -- * Types
    IdeState, TransitiveDependencies(..),
    -- * Functions
    runAction,
    toIdeResult,
    defineNoFile,
...
instance Pretty Log where
  pretty = \case
    ...
    LogDebugging s -> "Banana log" <+> pretty s
...
typeCheckRule recorder = define (cmapWithPrio LogShake recorder) $ \TypeCheck file -> do
    r <- use GetModificationTime file
    logWith recorder Logger.Info $ LogDebugging "in typechecking"
    let lspDiagnostic =
          Diag.Diagnostic {
            _range = noRange,
            _severity = Just DiagnosticSeverity_Warning,
            _code = Nothing,
            _source = Just "TypeCheck",
            _message = "IN TYPECHECKING ABNANA",
            _relatedInformation = Nothing,
            _tags = Nothing,
            _codeDescription = Nothing,
            _data_ = Nothing
          }
    pure ([FileDiagnostic file ShowDiag lspDiagnostic NoStructuredMessage] , Just ())
...
mainRule recorder RulesConfig{..} = do
    ...
    -- defineNoDiagnostics (cmapWithPrio LogShake recorder) $ \NeedsCompilation _ -> return $ Just Nothing
